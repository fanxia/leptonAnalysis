...
class BtagWeight {
 ...
 BtagWeight(int jmin) :
  ...
  bool filter(int t, bool nBtagInclusive) { return ((nBtagInclusive &&  t>= minTags) || (!nBtagInclusive && t == minTags)); }
  pair<float, float> weight(vector<BtagInfo> info, int tags, double scale, bool doScale, bool nBtagInclusive);

 private:
  int minTags;
};

pair<float, float> BtagWeight::weight(vector<BtagInfo> info, int tags, double scale, bool doScale, bool nBtagInclusive) {
  
  if(!filter(tags, nBtagInclusive)) return make_pair(0., 0.);
  ...
  for(int i = 0; i < comb; i++) {
    ...
    if(filter(ntagged, nBtagInclusive)) {
      pMC += mc;
      pMC_err2 += mc_err2;
      pData += data;
